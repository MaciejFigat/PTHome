...
app.get(`/lexapi/search/`, async (req: any, res: any, next) => {
  ...
  axios
    .get(`https://null.turbo-lex.pl/search/?query=${searchquery}`)
    .then(response => {
...
app.get(`/lexapi/doc/`, async (req: any, res: any, next) => {
  ...
  axios
    .get(
      `https://null.turbo-lex.pl/doc/${docNumber}?query_doc=${query}&selected_doc=${selectedDoc}`
    )
...
app.get(`/lexapi/doc/id/query`, async (req: any, res: any, next) => {
  ...
  axios
    .get(`https://null.turbo-lex.pl/doc/${docNumber}?query_doc=${query}`)
    // axios.get(`https://null.turbo-lex.pl/doc/${docNumber}?query=${query}&selected=${selectedDoc}`)
    .then(response => {
      res.json(JSON.parse(JSON.stringify(response.data)))
    })
...
app.get(`/lexapi/searchSkip/`, async (req: any, res: any, next) => {
  ...
  axios
    .get(
      `https://null.turbo-lex.pl/searchSkip/?query=${query}&skip=${skip}&take=${take}&start_date=${start_date}&end_date=${end_date}`
    )
...
app.get(`/lexapi/doc/nr`, async (req: any, res: any, next) => {
  axios
    .get(`https://null.turbo-lex.pl/doc/${req.query.nr}`)
...
app.post('/lexapi/feedback', (req, res) => {
  const { remote_addr, search_uuid, useful, query, accuracy } = req.body

  if (
    typeof remote_addr !== 'string' ||
    typeof search_uuid !== 'string' ||
  ...
  axios
    .post('https://null.turbo-lex.pl/feedback/', req.body)
    .then(response => {
